...
2. Prefer current `elementRef` values with `tap`, `type_text`, `long_press`, or other semantic actions. Use coordinates only when the hierarchy exposes no usable target.
...
Use `gesture` or scoped swipe actions when needed. If a gesture is unreliable, return to a known route or relaunch rather than repeatedly guessing coordinates.
